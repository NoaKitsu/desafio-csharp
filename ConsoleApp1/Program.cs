    ...
    public class Program{
        static void Main(string[] args){
            if(args == null || args.Length < 3){
                ...
                string filePath = Path.Combine(Directory.GetCurrentDirectory(), "ConsoleApp1/emailConfigs.txt");
                ...
                while(true){//verifica continuinamente a cada 1 minuto
                    string valueClose = QueryOperations(queryUri);
                    if(valueClose != null)
                        priceOperations(valueClose, valor_venda, valor_compra, emailDestiny, Provedor, Username, Password);
        ...
        static string QueryOperations(Uri queryUri){
        ...
        static void priceOperations(string valueClose, double valor_venda, double valor_compra, string? emailDestiny, string? Provedor, string? Username, string? Password){
